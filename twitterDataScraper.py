...
def main(argv):
    ...
    # Verify key/secrets
    if e["consumer_key"] is "CONSUMER_KEY" or e['consumer_secret'] is "CONSUMER_SECRET" or e[
        "access_token_key"] is "ACCESS_TOKEN" or e['access_token_secret'] is "ACCESS_TOKEN_SECRET":
        print('[ERROR] It seems that Twitter key/secrets where not set.')
        print(
            '\tGenerate here https://developer.twitter.com/en/docs/basics/authentication/guides/access-tokens.html and set to start playing')
        print('')
        print_usage()
        sys.exit(2)
    ...
    for tweet in get_tweets(file_in):
        if tweet_url is not None:
            ...
            for reply in get_replies(tweet):
                ...
                human_date = datetime.strptime(pre_human_date, '%a %b %d %H:%M:%S %Y').replace(
                    tzinfo=timezone('UTC')).astimezone(tz=timezone(local_timezone)).strftime('%d/%m/%Y %H:%M:%S')
                count += 1
                ...
                if short_output: print('%s,https://twitter.com/%s/status/%s,%s' % (
                human_date, j['user']['screen_name'], j['id'], tweet_url))
